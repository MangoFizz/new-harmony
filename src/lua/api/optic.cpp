...
namespace Harmony::Lua {
    using QuadraticBezier = Balltze::Math::QuadraticBezier;
    ...
    static int lua_create_animation(lua_State *state) noexcept {
        ...
        auto *optic = script->optic_container();
        ...
        if(args == 4 || args == 7) {
            ...
            if(anim) {
                ...
                if(args == 4) {
                    ...
                    curve = QuadraticBezier({x1, y1}, {x2, y2});
                }
    ...
    static int lua_create_sprite(lua_State *state) noexcept {
        ...
        int args = lua_gettop(state);
        if(args == 1) {
            std::string sprite_path = luaL_checkstring(state, 1);
            auto handle = optic->create_sprite(sprite_path);
            lua_pushinteger(state, handle);
            return 1;
        }
        else {
    ...
    static int lua_create_render_queue(lua_State *state) noexcept {
        ...
        if(args >= 6 && args <= 9) {
            ...
            initial_state.color.alpha = opacity;
    ...
    static int lua_create_sound(lua_State *state) noexcept {
        ...
        int args = lua_gettop(state);
        if(args == 1) {
            std::string sound_path = luaL_checkstring(state, 1);
    ...
    static int lua_create_sound_playback_queue(lua_State *state) noexcept {
        auto *script = library->get_script(state);
        auto *optic = script->optic_container();

        int args = lua_gettop(state);
        if(args == 0) {    
            auto handle = optic->create_sound_playback_queue();
            lua_pushinteger(state, handle);
            return 1;
        }
        else {
            luaL_error(state, "invalid number of arguments in harmony create_sound_playback_queue function");
        }
        return 0;
    }
    ...
    static int lua_clear_sound_playback_queue(lua_State *state) noexcept {
        auto *script = library->get_script(state);
        auto *optic = script->optic_container();
        ...
        if(args == 1) {    
            ...
            auto *playback_queue = optic->get_sound_playback_queue(playback_queue_handle);
            if(playback_queue) {
                playback_queue->clear();
            }
            else {
                luaL_error(state, "invalid audio engine handle in harmony clear_sound_playback_queue function");
            }
        }
        else {
            luaL_error(state, "invalid number of arguments in harmony clear_sound_playback_queue function");
        }
        return 0;
    }
    ...
    static const luaL_Reg optic[] = {
        ...
        {"create_sound_playback_queue", lua_create_sound_playback_queue},
        {"play_sound", lua_play_sound},
        {"clear_sound_playback_queue", lua_clear_sound_playback_queue},
        {NULL, NULL}
    };

    void set_optic_functions(lua_State *state) noexcept {
        library = &Library::get();
        optic_handler = &Optic::Handler::get();
